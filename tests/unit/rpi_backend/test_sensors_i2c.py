...
async def test_bme280_sensor():
    ...
    try:
        ...
        # Drive the real smbus2 code path with MockSMBus, whether or not smbus2 is installed
        with patch('hardware.sensors_i2c.smbus2', create=True) as mock_smbus_module, \
             patch('hardware.sensors_i2c.SMBUS_AVAILABLE', True), \
             patch('hardware.sensors_i2c.I2CInterface') as mock_i2c_class:

            mock_bus = MockSMBus(1)
            mock_smbus_module.SMBus.return_value = mock_bus
...
async def test_i2c_interface():
    ...
    try:
        ...
        with patch('hardware.sensors_i2c.I2CInterface.open'):
