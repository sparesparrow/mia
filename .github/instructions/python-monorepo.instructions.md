...
- Keep optional integrations optional. Many Python surfaces in `orchestration/mcp/modules/` and `tools/scripts/` are written to survive missing audio, serial, Docker, BLE, or vendor SDK dependencies; prefer guarded imports and degraded behavior over hard startup failures.
