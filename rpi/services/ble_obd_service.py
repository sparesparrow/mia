...
try:
    ...
    logging.warning("Aioble not available. BLE service disabled.")
...
class BLEOBDService:
    ...
    def __init__(self,
        ...
        # Characteristics
        self.rpm_char = None
        self.speed_char = None
        self.coolant_char = None
    ...
    async def setup_ble_services(self):
        ...
        if not AIOBLE_AVAILABLE:
            logger.error("Aioble not available. Cannot setup BLE services.")
            return False

        try:
            # Create OBD service
            self.obd_service = service.Service(OBD_SERVICE_UUID)

            # Create characteristics
            self.rpm_char = characteristic.Characteristic(
                self.obd_service,
                RPM_CHARACTERISTIC_UUID,
                read=True,
                notify=True,
                initial=self._encode_rpm(self.obd_data["rpm"])
            )

            self.speed_char = characteristic.Characteristic(
                self.obd_service,
                SPEED_CHARACTERISTIC_UUID,
                read=True,
                notify=True,
                initial=self._encode_speed(self.obd_data["speed"])
            )

            self.coolant_char = characteristic.Characteristic(
                self.obd_service,
                COOLANT_CHARACTERISTIC_UUID,
                read=True,
                notify=True,
                initial=self._encode_temp(self.obd_data["coolant_temp"])
            )

            self.command_char = characteristic.Characteristic(
                self.obd_service,
            ...
            self.response_char = characteristic.Characteristic(
                self.obd_service,
            ...
            # Register services
            aioble.register_services(self.obd_service)
    ...
    async def handle_command_write(self, conn):
    ...
    async def handle_command_write(self, conn):
        """Handle OBD command writes"""
        try:
            # Read command from characteristic
            command_bytes = await self.command_char.read()
            command = self._decode_command(command_bytes)
            ...
            # Update response characteristic
            await self.response_char.write(response.encode('utf-8'))
    ...
    async def peripheral_handler(self, conn):
        """Handle peripheral connections"""
        logger.info(f"BLE connection from: {conn}")

        try:
            # Subscribe to command writes
            await self.command_char.subscribe(self.handle_command_write)
            ...
            logger.error(f"Peripheral handler error: {e}")
        finally:
            logger.info(f"BLE disconnection: {conn}")
    ...
    async def start(self):
        ...
        if AIOBLE_AVAILABLE:
            try:
                logger.info(f"BLE peripheral starting with device name: {self.device_name}")
                ...
                # Start BLE advertising and handle connections
                async with aioble.peripheral() as peripheral:
