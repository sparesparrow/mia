...
try:
    # Import new comprehensive FlatBuffers bindings
    ...
    import Mia.SystemStatus as SystemStatus
    import Mia.CommandAck as CommandAck
    import Mia.CommandStatus as CommandStatus
    import Mia.LEDState as LEDState
    import Mia.LEDMode as LEDMode
    import Mia.AIState as AIState
    import Mia.DeviceInfo as DeviceInfo
    import Mia.DeviceType as DeviceType
    ...
    logger.info("FlatBuffers bindings loaded successfully")
    ...
    logger.warning(f"Could not import Mia FlatBuffers bindings ({e}). Using JSON-only mode.")
    ...
    try:
        ...
        while True:
            try:
                ...
                if VehicleTelemetry:
                    # Decode new FlatBuffers VehicleTelemetry message
                    telemetry = VehicleTelemetry.VehicleTelemetry.GetRootAs(msg, 0)
                    ...
                    data = {
                        ...
                        # DPF data
                        "dpf_soot_load_percent": round(telemetry.DpfSootLoadPercent(), 1),
                ...
                elif CitroenTelemetry:
                    # Fallback to legacy CitroenTelemetry message
...
async def get_status():
    ...
    try:
        ...
        return {
            ...
            "devices_connected": len(device_registry.get_all()) if REGISTRY_AVAILABLE and device_registry else 0,
            "timestamp": datetime.now().isoformat()
        }
    except Exception as e:
...
async def configure_gpio(gpio: GPIOCommand):
    ...
    try:
        if GPIOCommand and FLATBUFFERS_AVAILABLE:
            ...
            GPIOCommand.GPIOCommandAddPin(builder, gpio.pin)
            GPIOCommand.GPIOCommandAddDirection(builder, getattr(GPIOCommand, f"GPIODirection_{gpio.direction.title()}") if hasattr(GPIOCommand, f"GPIODirection_{gpio.direction.title()}") else GPIOCommand.GPIODirection_Output)
            GPIOCommand.GPIOCommandAddValue(builder, gpio.value or False)
            GPIOCommand.GPIOCommandAddTimestamp(builder, int(datetime.now().timestamp() * 1000000))
            fb_message = GPIOCommand.GPIOCommandEnd(builder)
            builder.Finish(fb_message)

            # Send FlatBuffers message via ZeroMQ
            zmq_socket.send(b"GPIO_CONFIG", zmq.SNDMORE)  # Message type
            zmq_socket.send(builder.Output())  # FlatBuffers payload
        else:
            # Fallback to JSON
            message = {
                "type": "GPIO_CONFIGURE",
        ...
        if poller.poll(5000):
            if GPIOCommand and FLATBUFFERS_AVAILABLE:
                # Receive FlatBuffers response
                response_data = zmq_socket.recv()
                if GPIOResponse:
                    response = GPIOResponse.GPIOResponse.GetRootAs(response_data, 0)
                    return {
                        "success": response.Success(),
                        "pin": response.Pin(),
                        "value": response.Value(),
                        "error": response.ErrorMessage().decode('utf-8') if response.ErrorMessage() else None
                    }
                else:
                    return {"success": True, "response": "GPIO configured (FlatBuffers not available)"}
            else:
                response = zmq_socket.recv_json()
        ...
        logger.error(f"GPIO configure error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def set_gpio(gpio: GPIOCommand):
    ...
    try:
        if GPIOCommand:
            ...
            GPIOCommand.GPIOCommandStart(builder)
            GPIOCommand.GPIOCommandAddPin(builder, gpio.pin)
            GPIOCommand.GPIOCommandAddDirection(builder, GPIOCommand.GPIODirection_Output)
            GPIOCommand.GPIOCommandAddValue(builder, gpio.value or False)
            GPIOCommand.GPIOCommandAddTimestamp(builder, int(datetime.now().timestamp() * 1000000))
            fb_message = GPIOCommand.GPIOCommandEnd(builder)
            builder.Finish(fb_message)
            ...
            # Send FlatBuffers message via ZeroMQ
            zmq_socket.send(b"GPIO_SET", zmq.SNDMORE)  # Message type
            zmq_socket.send(builder.Output())  # FlatBuffers payload
        else:
            # Fallback to JSON
            message = {
...
async def get_gpio(pin: int):
    ...
    try:
        if GPIOCommand:
            # Use FlatBuffers message for request
            import flatbuffers
            builder = flatbuffers.Builder(256)

            GPIOCommand.GPIOCommandStart(builder)
            GPIOCommand.GPIOCommandAddPin(builder, pin)
            GPIOCommand.GPIOCommandAddDirection(builder, GPIOCommand.GPIODirection_Input)
            GPIOCommand.GPIOCommandAddValue(builder, False)
            GPIOCommand.GPIOCommandAddTimestamp(builder, int(datetime.now().timestamp() * 1000000))
            fb_message = GPIOCommand.GPIOCommandEnd(builder)
            builder.Finish(fb_message)

            # Send FlatBuffers message via ZeroMQ
            zmq_socket.send(b"GPIO_GET", zmq.SNDMORE)  # Message type
            zmq_socket.send(builder.Output())  # FlatBuffers payload
        else:
            # Fallback to JSON
            message = {
...
# ============================================================================
# LED Control Endpoints (Phase 4.4)
# ============================================================================

@app.post("/led/ai_state")
async def set_led_ai_state(ai_state: str, mode: str = "Drive", brightness: float = 1.0):
    """
    POST /led/ai_state - Set AI state for LED display

    Controls the LED indicators based on AI assistant state.
    """
    try:
        if LEDState and AIState and LEDMode:
            # Use FlatBuffers message
            import flatbuffers
            builder = flatbuffers.Builder(512)

            # Convert string parameters to enum values
            ai_state_enum = getattr(AIState, f"AIState_{ai_state.title()}", AIState.AIState_Idle)
            mode_enum = getattr(LEDMode, f"LEDMode_{mode.title()}", LEDMode.LEDMode_Drive)

            # Create service status (placeholder for now)
            service_status = builder.CreateString("api:healthy,gpio:healthy,obd:unknown")

            LEDState.LEDStateStart(builder)
            LEDState.LEDStateAddMode(builder, mode_enum)
            LEDState.LEDStateAddAiState(builder, ai_state_enum)
            LEDState.LEDStateAddBrightness(builder, brightness)
            LEDState.LEDStateAddServiceStatus(builder, service_status)
            # OBD data would be added here when available
            LEDState.LEDStateAddTimestamp(builder, int(datetime.now().timestamp() * 1000000))
            fb_message = LEDState.LEDStateEnd(builder)
            builder.Finish(fb_message)

            # Send via ZeroMQ
            zmq_socket.send(b"LED_AI_STATE", zmq.SNDMORE)
...
@app.post("/led/service_status")
async def update_led_service_status(services: Dict[str, str]):
    """
    POST /led/service_status - Update service health status for LED display

    Services should be a dict like {"api": "healthy", "gpio": "error", "obd": "unknown"}
    """
    try:
        if LEDState:
            # Use FlatBuffers message
            ...
            # Create service status string
            service_status_str = ",".join(f"{k}:{v}" for k, v in services.items())
            service_status = builder.CreateString(service_status_str)

            LEDState.LEDStateStart(builder)
            LEDState.LEDStateAddMode(builder, LEDMode.LEDMode_Service)
            LEDState.LEDStateAddAiState(builder, AIState.AIState_Idle)
            LEDState.LEDStateAddBrightness(builder, 1.0)
            LEDState.LEDStateAddServiceStatus(builder, service_status)
            LEDState.LEDStateAddTimestamp(builder, int(datetime.now().timestamp() * 1000000))
            fb_message = LEDState.LEDStateEnd(builder)
            builder.Finish(fb_message)

            zmq_socket.send(b"LED_SERVICE_STATUS", zmq.SNDMORE)
            zmq_socket.send(builder.Output())
        else:
            # Fallback to JSON
            message = {
                "type": "LED_SERVICE_STATUS",
                "services": services,
                "timestamp": datetime.now().isoformat()
            }
            zmq_socket.send_json(message)
        ...
        logger.error(f"LED service status error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/led/obd_data")
async def send_led_obd_data(obd_data: Dict[str, Any]):
    """
    POST /led/obd_data - Send OBD telemetry data for LED visualization

    OBD data should include rpm, speed, temperature, load, etc.
    """
    try:
        if LEDState:
            # Use FlatBuffers message with OBD data
            import flatbuffers
            import json
            builder = flatbuffers.Builder(1024)

            # Serialize OBD data as JSON bytes for now
            obd_json = json.dumps(obd_data).encode('utf-8')
            obd_bytes = builder.CreateByteVector(obd_json)

            service_status = builder.CreateString("obd:active")

            LEDState.LEDStateStart(builder)
            LEDState.LEDStateAddMode(builder, LEDMode.LEDMode_Drive)
            LEDState.LEDStateAddAiState(builder, AIState.AIState_Idle)
            LEDState.LEDStateAddBrightness(builder, 1.0)
            LEDState.LEDStateAddServiceStatus(builder, service_status)
            LEDState.LEDStateAddObdData(builder, obd_bytes)
            LEDState.LEDStateAddTimestamp(builder, int(datetime.now().timestamp() * 1000000))
            fb_message = LEDState.LEDStateEnd(builder)
            builder.Finish(fb_message)

            zmq_socket.send(b"LED_OBD_DATA", zmq.SNDMORE)
            zmq_socket.send(builder.Output())
        else:
            # Fallback to JSON
            message = {
...
@app.post("/led/mode")
async def set_led_mode(mode: str, brightness: float = 1.0):
    """
    POST /led/mode - Change LED system mode

    Modes: Drive, Parked, Night, Service, Emergency
    """
    try:
        if LEDState and LEDMode:
            ...
            mode_enum = getattr(LEDMode, f"LEDMode_{mode.title()}", LEDMode.LEDMode_Drive)
            service_status = builder.CreateString("mode:changed")
            ...
            zmq_socket.send(b"LED_MODE", zmq.SNDMORE)
            zmq_socket.send(builder.Output())
        else:
            # Fallback to JSON
            message = {
...
async def get_led_status():
    """
    GET /led/status - Get current LED display status
    """
    try:
        # Request current LED status
        message = {
        ...
        if poller.poll(5000):
            ...
            return {
                "success": False,
                "error": "LED status request timeout",
                "timestamp": datetime.now().isoformat()
            }
    except Exception as e:
