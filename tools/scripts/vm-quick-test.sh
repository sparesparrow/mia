...
quick_project_test() {
    ...
    local scripts=("tools/scripts/dev-environment.sh" "tools/scripts/health-check.sh")
...
main() {
    ...
    if [ $tests_passed -eq $tests_total ]; then
        ...
        echo ""
        echo "Next steps:"
        echo "  1. Run full test: ./tools/scripts/vm-test-setup.sh"
        echo "  2. Start development: ./tools/scripts/dev-environment.sh up dev"
...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    ...
    echo ""
    echo "For full testing, use: ./tools/scripts/vm-test-setup.sh"
    exit 0
fi
