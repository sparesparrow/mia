...
build_automotive_containers() {
    ...
    if [[ -x "$PROJECT_ROOT/tools/scripts/docker-orchestration.sh" ]]; then
        AUTOMOTIVE_MODE=true EDGE_OPTIMIZATION=true \
        "$PROJECT_ROOT/tools/scripts/docker-orchestration.sh" build
    else
...
deploy_monitoring() {
    ...
    if [[ -f "$PROJECT_ROOT/infra/monitoring/prometheus-automotive.yml" ]]; then
        docker run -d \
            --name ai-servis-prometheus \
            --restart unless-stopped \
            -p 9090:9090 \
            -v "$PROJECT_ROOT/infra/monitoring/prometheus-automotive.yml:/etc/prometheus/prometheus.yml" \
            -v "$PROJECT_ROOT/infra/monitoring/rules:/etc/prometheus/rules" \
...
deploy_with_compose() {
    ...
    case "$DEPLOYMENT_ENV" in
        ...
        monitoring)
            ...
            ;;
        pi-sim)
            compose_file="tests/env/docker-compose.pi-simulation.yml"
            ;;
        *)
...
run_performance_tests() {
    ...
    if [[ -x "$PROJECT_ROOT/tools/scripts/automotive-performance-test.py" ]]; then
        cd "$PROJECT_ROOT"
        
        # Wait for services to be fully ready
        sleep 30
        
        python3 tools/scripts/automotive-performance-test.py \
