...
test_development_environment() {
    ...
    timeout 300s ./tools/scripts/dev-environment.sh up dev --build || {
...
test_pi_simulation() {
    ...
    timeout 300s ./tools/scripts/dev-environment.sh up pi-sim --build || {
    ...
    # Stop Pi simulation
    ./tools/scripts/dev-environment.sh down pi-sim
...
test_monitoring_stack() {
    ...
    timeout 300s ./tools/scripts/dev-environment.sh up monitoring --build || {
    ...
    ./tools/scripts/dev-environment.sh down monitoring
...
test_ci_simulation() {
    ...
    if [ -f "tools/scripts/docker-build-multiplatform.sh" ]; then
        log "Testing multi-platform build script..."
        if timeout 60s ./tools/scripts/docker-build-multiplatform.sh --help >/dev/null; then
...
test_performance_validation() {
    ...
    ./tools/scripts/dev-environment.sh up dev >/dev/null 2>&1
    sleep 30
    
    # Run performance tests if available
    if [ -f "tools/scripts/performance-tests.sh" ]; then
        log "Running performance tests..."
        if timeout 120s ./tools/scripts/performance-tests.sh; then
    ...
    # Stop environment
    ./tools/scripts/dev-environment.sh down dev >/dev/null 2>&1
