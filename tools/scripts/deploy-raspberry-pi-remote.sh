...
"${SSH_BASE_CMD[@]}" "${SSH_TARGET[@]}" "mkdir -p ${RPI_PATH}/{modules/hardware-bridge,apps/arduino/led_strip_controller,scripts}"
...
rsync -avz -e "${RSYNC_SSH_CMD[*]}" \
    apps/arduino/led_strip_controller/ \
    ${RPI_USER}@${RPI_HOST}:${RPI_PATH}/apps/arduino/led_strip_controller/
