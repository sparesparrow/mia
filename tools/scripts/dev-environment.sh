...
ENVIRONMENTS=(
    ...
    "pi-sim:tests/env/docker-compose.pi-simulation.yml:Raspberry Pi simulation environment"
...
PI_SIM_COMPOSE_FILE="$PROJECT_ROOT/tests/env/docker-compose.pi-simulation.yml"
...
health_check() {
    ...
    if [ -f "$PROJECT_ROOT/tools/scripts/health-check.sh" ]; then
        "$PROJECT_ROOT/tools/scripts/health-check.sh"
    else
...
run_tests() {
    ...
    case "$env" in
        "dev"|"full")
            if [ -f "$PROJECT_ROOT/tools/scripts/system-tests.sh" ]; then
                "$PROJECT_ROOT/tools/scripts/system-tests.sh"
            fi
            ;;
        "pi-sim")
